...
def extract_answer(Q, text, max_a_len=16):
    """抽取答案函数
    """
    max_q_len = 131
    q_token_ids = tokenizer.encode(Q, max_length=max_q_len)[0]
    c_token_ids = tokenizer.encode(
        text, max_length=maxlen - len(q_token_ids) + 1
    )[0]
    token_ids = q_token_ids + c_token_ids[1:]
    segment_ids = [0] * len(q_token_ids) + [1] * (len(c_token_ids) - 1)
    c_tokens = tokenizer.tokenize(text)[1:-1]
    mapping = tokenizer.rematch(text, c_tokens)
        ...
        for end, p_end in enumerate(probas[1]):
            if end >= start and end < start + max_a_len:
                if p_start * p_end > score:
                    ...
                    score = p_start * p_end
    start, end = start_end
    return text[mapping[start][0]:mapping[end][-1] + 1]
